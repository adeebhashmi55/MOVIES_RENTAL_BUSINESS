...
-- How many movies are with rehtal rate of $0.99? --
...
--  REVENUE PER FILM (TOP 10 GROSSES)
...
SELECT MONTHNAME(RENTAL_DATE) AS MONTH_NAME,EXTRACT(YEAR FROM RENTAL_DATE) AS YEAR_NUMBER, COUNT(RENTAL.RENTAL_ID)
FROM RENTAL
GROUP BY EXTRACT(YEAR FROM RENTAL_DATE),MONTHNAME(RENTAL_DATE);

-- REWARDS USERS WHO HAVE RENTED AT LEAST 30 TIMES(WITH DETAIL OF CUSTOMERS)
...
SELECT LOYAL_CUSTOMERS.CUSTOMER_ID,C.FIRST_NAME,C.LAST_NAME,C.EMAIL,AD.ADDRESS_ID,PHONE
